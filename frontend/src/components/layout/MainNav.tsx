      ...
      <div className="main-nav__inner">
        ...
        <div className="main-nav__actions">
          {user ? (
            <Link
              href="/account"
              className="main-nav__icon main-nav__avatar"
              aria-label={accountAriaLabel(user)}
              data-testid="nav-account"
              title={user.full_name?.trim() || user.email}
            >
              <span className="main-nav__avatar-text" aria-hidden="true">
                {userInitials(user)}
              </span>
            </Link>
          ) : (
...
function userInitials(user: { full_name?: string | null; email: string }): string {
...
function accountAriaLabel(user: { full_name?: string | null; email: string }): string {
  const name = user.full_name?.trim();
  return name ? `Account, ${name}` : `Account, ${user.email}`;
}
